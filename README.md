...
- `src/main/server`: Electron utility-process supervisor for port selection, readiness probing, redacted rotating logs, bounded restart, and graceful shutdown. Electron main owns only window/session/native integration; Git, filesystem, persistence, AI, GitHub, and watchers remain in the utility server.

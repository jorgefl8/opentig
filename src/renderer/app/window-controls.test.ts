...
describe('resolveWindowControlsInset', () => {
  it('measures the space left and right of the reported title bar area', () => {
    expect(resolveWindowControlsInset({ rect: { x: 0, width: 1062, height: 46 }, viewportWidth: 1200, mac: false }))
      .toEqual({ left: 0, right: 138, height: 46 });
    expect(resolveWindowControlsInset({ rect: { x: 78, width: 1122, height: 46 }, viewportWidth: 1200, mac: true }))
  ...
  it('ignores an empty title bar area and never returns negative insets', () => {
    expect(resolveWindowControlsInset({ rect: { x: 0, width: 0, height: 46 }, viewportWidth: 1200, mac: false }))
      .toEqual({ left: 0, right: 138, height: 0 });
    expect(resolveWindowControlsInset({ rect: { x: 0, width: 1400, height: 46 }, viewportWidth: 1200, mac: false }))

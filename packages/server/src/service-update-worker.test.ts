...
const manager = vi.hoisted(() => ({ unit: '', restart: vi.fn() }));
vi.mock('./service-manager', async (original) => ({
  ...await original<typeof import('./service-manager')>(),
  createServiceManager: async () => ({
    kind: 'systemd',
    read: () => readFile(manager.unit, 'utf8'),
    owns: () => true,
    write: (definition: string) => writeFile(manager.unit, definition),

...
describe('OpenTig CLI arguments', () => {
  it('defaults to start with the current directory and loopback endpoint', () => {
    expect(parseCliArguments([], {}, cwd, home)).toEqual({
      command: 'start', cwd, host: '127.0.0.1', port: 6767, portExplicit: false,
  ...
  it('gives flags precedence over environment and records explicit ports', () => {
    expect(parseCliArguments(
      ['--host=192.168.1.4', '--port', '7000', '--home', 'state'],
      { OPENTIG_HOST: '10.0.0.2', OPENTIG_PORT: '7001', OPENTIG_HOME: 'other' }, cwd, home,
    )).toMatchObject({ host: '192.168.1.4', port: 7000, portExplicit: true, home: path.join(cwd, 'state') });
    expect(parseCliArguments([], { OPENTIG_PORT: '7001' }, cwd, home)).toMatchObject({ port: 7001, portExplicit: true });
  });

...
export function createApi(ipcRenderer: IpcRenderer, setZoomFactor: (factor: number) => void): OpenTigApi {
  ...
  return {
    ...
    clipboard: {
      readFilePaths: () => invoke(IPC.clipboardReadFilePaths),
      readImagePng: () => invoke(IPC.clipboardReadImagePng),
    },
    repository: {
      select: (title) => title === undefined ? invoke(IPC.repositorySelect) : invoke(IPC.repositorySelect, title),
      selectRelocation: (repositoryName, previousPath) => invoke(IPC.repositorySelectRelocation, repositoryName, previousPath),

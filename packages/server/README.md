...
`npx` and `bunx` never install background persistence. Linux/systemd users can opt in with `opentig service install [cwd]`, inspect it with `opentig service status`, and remove it with `opentig service uninstall`. The installer stages the exact package outside the temporary npx cache and enables user lingering. Other platforms should use their preferred service manager for now.
